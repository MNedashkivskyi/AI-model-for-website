...
def prepare_data(data_begin, data_end):
    ...
    for i, date in enumerate(dates):
        ...
        df = pd.merge(buying_events_first_month, buying_events_second_month, how='left',
        ...
        spent_money_first_month = deals[
            (deals.timestamp.dt.month == first_month) & (deals.timestamp.dt.year == first_year)].groupby(
            'user_id').aggregate(
            {'final_price': 'sum'}).rename(columns={'final_price': 'spent_money_first_month'}).reset_index()

        spent_money_second_month = deals[
            (deals.timestamp.dt.month == second_month) & (deals.timestamp.dt.year == second_year)].groupby(
            'user_id').aggregate(
        ...
        spent_money_third_month = deals[
            (deals.timestamp.dt.month == third_month) & (deals.timestamp.dt.year == third_year)].groupby(
            'user_id').aggregate(
            {'final_price': 'sum'}).rename(
        ...
        spent_money_average['label'] = np.where(
            spent_money_average['spent_money_third_month'] - spent_money_average['spent_money_total_average'] > 100, 1,
            0)
        processed_data = pd.merge(df, spent_money_average[
            ['spent_money_first_month', 'spent_money_second_month', 'user_id', 'label']], how='left', on=['user_id'])
